...
def solve(operations, budget, simplicity):
    ...
    operations = [op for op in operations if not set(
        op.inputs.keys()).difference(outputs)]
    operations = [op for op in operations if op.profit >
                  0 or set(op.outputs.keys()).intersection(inputs)]
    ...
    prob += lpSum(op.cost *
                  op.lpvariable for op in operations if op.cost) <= budget
    if simplicity:
    ...
    solution = prob.solve(
        COIN_CMD('D:\\cbc\\bin\\cbc.exe', msg=1, **options.solveroptions))
